...
Production deployments should pin a version tag; `latest` always means the newest release (only tags publish images), never the tip of `main`. The image is a two-stage build (`docker/Dockerfile`: Rust builder →

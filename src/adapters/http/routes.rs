...
pub fn create_router(state: Arc<AppState>, api_key: Option<String>) -> Router<()> {
    let api_routes = Router::new()
        ...
        .route("/api/v1/sources/{id}/dataset", get(http::sources::get_source_dataset))
        .route("/api/v1/sources/{id}/sync", post(http::sync::sync_source))
        .route("/api/v1/sources/{id}/status", get(http::sources::source_status))
        .route("/api/v1/sources/{id}/hosts/{hostname}", put(http::hosts::put_host).delete(http::hosts::delete_host))
        .route("/api/v1/enrichers/{id}/run", post(http::enrichers::run_enricher))
        .route("/api/v1/endpoints", get(http::endpoints::list_endpoints))
        .route("/api/v1/endpoints/{id}", post(http::endpoints::run_endpoint))
